...
<div class="container-fluid py-4">
    ...
    <div class="row mb-4">
        <div class="col">
            ...
            <p class="text-muted">Tambah pengiriman untuk pesanan pelanggan</p>
        </div>
    </div>
    ...
    <div class="card shadow-sm">
        <div class="card-header bg-primary text-white">
            <h5 class="mb-0"><i class="bi bi-clipboard-plus me-2"></i>Form Pengiriman</h5>
        </div>
        <div class="card-body">
            <form action="{{ route('pengiriman.store') }}" method="POST" id="formPengiriman">
                ...
                <div class="row g-3">
                    {{-- Pilih Pesanan (Kontekstual) --}}
                    <div class="col-md-6">
                        <label for="id_pesanan" class="form-label fw-bold">
                            Pilih Pesanan <span class="text-danger">*</span>
                        </label>
                        <select class="form-select @error('id_pesanan') is-invalid @enderror" 
                                id="id_pesanan" 
                                name="id_pesanan" 
                                required>
                            ...
                            @foreach($pesanans as $pesanan)
                                <option value="{{ $pesanan->id_pesanan }}" 
                                        data-pelanggan="{{ $pesanan->pelanggan->nama ?? 'N/A' }}"
                                        data-no-hp="{{ $pesanan->pelanggan->no_hp ?? 'N/A' }}"
                                        data-alamat="{{ $pesanan->pelanggan->alamat ?? 'N/A' }}">
                                    #PSN-{{ str_pad($pesanan->id_pesanan, 4, '0', STR_PAD_LEFT) }} | 
                                    {{ $pesanan->pelanggan->nama ?? 'N/A' }} | 
                                    {{ $pesanan->detailPesanan->count() }} item | 
                    ...
                    {{-- Detail Pesanan (Auto-load, Read-only) --}}
                    <div class="col-12">
                        <div class="card bg-light" id="detailPesananCard" style="display: none;">
                            <div class="card-header bg-info text-white">
                                <h6 class="mb-0"><i class="bi bi-info-circle me-2"></i>Detail Pesanan</h6>
                            </div>
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-4">
                                        <p class="mb-1"><strong>Nama Pelanggan:</strong></p>
                                        <p id="detailNamaPelanggan" class="text-muted">-</p>
                                    </div>
                                    <div class="col-md-4">
                                        <p class="mb-1"><strong>No WhatsApp:</strong></p>
                                        <p id="detailNoHP" class="text-muted">-</p>
                                    </div>
                                    <div class="col-md-4">
                                        <p class="mb-1"><strong>Alamat Pesanan:</strong></p>
                                        <p id="detailAlamat" class="text-muted">-</p>
                                    </div>
                                </div>
                                <div class="row mt-2">
                                    <div class="col-12">
                                        <p class="mb-1"><strong>Produk & Jumlah:</strong></p>
                                        <div id="detailProduk" class="text-muted">-</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select @error('jenis_pengiriman') is-invalid @enderror" 
                            ...
                            <option value="Internal / Ambil Sendiri" {{ old('jenis_pengiriman') == 'Internal / Ambil Sendiri' ? 'selected' : '' }}>Internal / Ambil Sendiri</option>
                            <option value="Kurir Lokal" {{ old('jenis_pengiriman') == 'Kurir Lokal' ? 'selected' : '' }}>Kurir Lokal</option>
                            <option value="Ekspedisi" {{ old('jenis_pengiriman') == 'Ekspedisi' ? 'selected' : '' }}>Ekspedisi</option>
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select @error('status') is-invalid @enderror" 
                                ...
                                name="status" 
                                required>
                            <option value="Menunggu Dijadwalkan" {{ old('status', 'Menunggu Dijadwalkan') == 'Menunggu Dijadwalkan' ? 'selected' : '' }}>Menunggu Dijadwalkan</option>
                        ...
                        <div class="card">
                            <div class="card-body">
                                <p class="text-muted mb-2">
                                    <i class="bi bi-geo-alt me-1"></i>Klik pada peta untuk menentukan lokasi pengiriman
                                </p>
                    ...
                    <div class="col-12">
                        ...
                        <textarea class="form-control @error('catatan') is-invalid @enderror" 
                                  ...
                                  name="catatan" 
                                  rows="3" 
                                  placeholder="Catatan tambahan untuk pengiriman (opsional)">{{ old('catatan') }}</textarea>
                        @error('catatan')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="d-flex justify-content-end gap-2 mt-4 pt-3 border-top">
                    ...
                    <button type="submit" class="btn btn-primary">
                        <i class="bi bi-save2 me-1"></i>Simpan Pengiriman
                    </button>
                </div>
            </form>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    // Inisialisasi peta dengan koordinat default (Surabaya)
        ...
        fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lng}&addressdetails=1`)
            ...
            .then(data => {
                if (data && data.address) {
                    ...
                    // Update marker popup
                    if (marker) {
                        marker.bindPopup(`<b>Lokasi Pengiriman</b><br>${fullAddress}`).openPopup();
                    }
                }
            })
            .catch(error => {
    ...
    // ========================================
    // AUTO-LOAD DETAIL PESANAN
    // ========================================
    document.getElementById('id_pesanan').addEventListener('change', function() {
        const pesananId = this.value;
        const detailCard = document.getElementById('detailPesananCard');
        
        if (pesananId) {
            detailCard.style.display = 'block';
            
            fetch(`/api/pesanan/${pesananId}/detail`)
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        const pelanggan = data.data.pelanggan;
                        const produk = data.data.produk;
                        
                        document.getElementById('detailNamaPelanggan').textContent = pelanggan.nama;
                        document.getElementById('detailNoHP').textContent = pelanggan.no_hp;
                        document.getElementById('detailAlamat').textContent = pelanggan.alamat;
                        
                        let produkHTML = '<ul class="mb-0">';
                        produk.forEach(item => {
                            produkHTML += `<li>${item.nama} - ${item.jumlah} ${item.satuan}</li>`;
                        });
                        produkHTML += '</ul>';
                        document.getElementById('detailProduk').innerHTML = produkHTML;
                    }
                })
                .catch(error => {
